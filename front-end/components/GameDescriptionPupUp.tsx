...
const GameDescriptionPopup: React.FC<Props> = ({
  selectedGame,
  onClose,
  moderatorStatus,
  purchasedStatus,
  ...
  const [extraPermissions, setExtraPermissions] =
    useState<Boolean>(moderatorStatus);
  ...
  const checkGameInFavorites = async () => {
    try {
      ...
      if (username) {
        ...
        const isGameInFavorites = list.games.some(
  ...
  const handleListAdd = async () => {
    try {
      ...
      if (favoritedStatus) {
        ...
        // Add to favorites
        const { message } = await ListService.addGameToFavorites(
          username,
          gameId
        );
  ...
  if (error)
    return (
      <HintContainer error={new Error(error)} onClose={() => setError(null)} />
    );
          ...
          <>
            ...
            {!moderatorStatus && 
              <div className={styles.buttonsWrap}>
                ...
                {purchasedStatus && <p>Game has already been purchased</p>}
                <button className={styles.listButton} onClick={handleListAdd}>
