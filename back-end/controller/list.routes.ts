...
router.put('/add', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const updatedFavoritesList = await listService.addGameToFavorites(String(username), Number(gameId));

        // Return a success response with the updated list or a success message
        res.status(200).json({
